    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-end mt-4 flex-wrap gap-4">
            <div class="flex items-center gap-4">
                ...
                <a class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100" href="{{ url('/register') }}">
                    {{ __('Create Account') }}
                </a>
            </div>

            <x-primary-button class="ml-6 gap-x-6 gap-y-2">
                {{ __('Log in') }}
            </x-primary-button>
        </div>
    </form>
